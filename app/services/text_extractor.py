...
class TextExtractor:
    ...
    async def extract(file: UploadFile) -> str:
        """
        Extracts text from an uploaded file based on its file extension.
        """
        ...
        try:
            ...
            if not text.strip():
                raise FileExtractionError(
                    f"No text could be extracted from '{filename}'. The file might be empty, password-protected, or image-based."
                )

            logger.info(
                f"Successfully extracted text from '{filename}'. Word count: {len(text.split())}"
            )
            return text
